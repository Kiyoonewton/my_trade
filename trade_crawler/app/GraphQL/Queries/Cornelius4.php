      ...
      $matchResults = collect($matchSet['matches'])->map(function ($match) use ($seasons, $matchDays_array) {
        return OverOrUnder::where('season_id', $seasons)
          ...
          ->map(function ($item) {
            ...
            $sameOdd = $result === 1 ? $item->under > $item->over && $item->under : $item->over;
  ...
  public function __invoke($_, $args)
  {
    ...
    for ($j = $start; $j < $end; $j++) {
      ...
      for ($l = 1; $l < 561; $l++) {
        ...
        // return $this->getCount($team, $j, 1);

        // $countZeros = count(array_filter($this->getTotal($team, $j, 1), function ($value) {
        //   return $value === 0;
        // }));


        if ($this->getOneCount($team, $j, 1)[0] === 6) {
          // return $this->getOneCount($team,$j,1)[4];
          ...
          $result1 = implode(",", $this->getOneCount($team, $j, $winOutcome + 1)[4]);
          $matchday1 = implode(",", $this->getCount($team, $j, $winOutcome +1 )[3]);
          $sameOdd1 = implode(",", $this->getCount($team, $j, $winOutcome + 1)[1]);
